...
char *MYFILE=argv[1];
 ...
 // double theta13 = asin(sqrt(0.001))/2;
  ...
  for(y=-180;y<180.0+0.01;y=y+180.0/20)
  {
  glbSetOscParams(true_values,y*M_PI/180.0,GLB_DELTA_CP);
  glbSetOscillationParameters(true_values);
  glbSetRates();

	for(x = y-20;x<y+20;x=x+180.0/50){
      glbSetOscParams(test_values,x*M_PI/180.0,GLB_DELTA_CP);
  ...
  //glbSetOscillationParameters(true_values);
      ...
      res=glbChiDelta(test_values,fit_values,GLB_ALL);
      AddToOutput(y,x,sqrt(fabs(res)));
	}
