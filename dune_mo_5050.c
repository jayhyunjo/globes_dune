 ...
 * Figure 6(a): 50/50 FHC-RHC split
...
int main(int argc, char *argv[])
{
  ...
  glbInitExperiment("DUNE_GLoBES_fig6_5050.glb",&glb_experiment_list[0],&glb_num_of_exps);
  ...
  for(y=-180;y<180.0+0.01;y=y+180.0/50)
  {
    ...
    /* For True IO: delta_chi2 = chi2_wrong - chi2_right = chi2_NO - chi2_IO
     * But we want to plot it on the same y-axis as True NO (positive values = sensitivity)
     * Convention: delta_chi2 = chi2_IO - chi2_NO 
     * For True IO this will be NEGATIVE (IO fits better than NO when IO is true)
     * We report the absolute value to match the paper's convention */
